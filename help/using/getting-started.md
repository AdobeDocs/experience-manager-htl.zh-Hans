...
>[!NOTE]
>
>要运行本页上提供的大多数示例，可使用名为“读取评 [估打印循环](https://github.com/Adobe-Marketing-Cloud/aem-htl-repl) ”的实时执行环境。
...
即使在同一组件中，HTL文件也可以与JSP和ESP一起使用。 以下示例在 **第1行** （如何从JSP文件包含HTL文件）和第 **2行(如** 何从HTL文件包含JSP文件)中显示：
...
将输出类似于以下HTML的内容，但只有同时定义了 **`jcr:title`** 和属 **`jcr:description`** 性，并且两者都不为空：
...
以下示例在 **第1行** HTL注释和 **第2行** HTML注释中显示：
...
表达式只能放置到HTML文本或属性值中，但不能放在元素名称或属性名称中，否则将不再是有效的HTML。 为了动态设置元素名称，可 [`data-sly-element`](block-statements.md#element) 以在所需元素上使用该语句，并动态设置属性名称，甚至一次设置多个属性，也 [`data-sly-attribute`](block-statements.md#attribute) 可以使用该语句。
...
正如下面“自 [动上下文感知逃逸](getting-started.md#automatic-context-aware-escaping) ”一节所述，HTL的一个目标是通过自动将上下文感知逃逸应用于所有表达式，降低引入跨站点脚本(XSS)漏洞的风险。 虽然HTL可以自动检测置于HTML标记内的表达式的上下文，但它不分析内联JavaScript或CSS的语法，因此需要开发人员明确指定哪些上下文应用于此类表达式。
...
有关如何控制转义的详细信息，请参阅 [表达式语言显示上下文](expression-language.md#display-context) 。

### 特殊上下文的解除限制 {#lifting-limitations-of-special-contexts}

在需要绕过脚本、样式和注释上下文限制的特殊情况下，可以将其内容分离到单独的HTL文件中。 HTL会将位于其自己文件中的所有内容解释为普通HTML片段，而忽略可能包含该片段的限制上下文。

有关示 [例，请参阅使用客户端](getting-started.md#working-with-client-side-templates) “模板”一节。
...
* 该语句的部 [`data-sly-use` 分](block-statements.md#use) ，解释了使用该语句可以完成的所有操作。
* “ [Use-API](use-api.md) ”页提供一些信息，帮助您选择使用Java或JavaScript编写逻辑。
* 要详细说明如何编写逻辑， [JavaScript Use-API](use-api-javascript.md)[和Java Use-API页应该有所帮助](use-api-java.md) 。
...
在大多数模板语言中，此示例可能会创建跨站点脚本(XSS)漏洞，因为即使所有变量都自动以HTML形式转义，该属 `href` 性也必须特别以URL形式转义。 这种疏漏是最常见的错误之一，因为它很容易被遗忘，而且很难以自动方式发现。
...
请注意两个属性是如何以不同方式转义的，因为HTL知道 `href` URI上 `src` 下文必须转义和属性。 此外，如果URI是从开 **`javascript:`**&#x200B;始的，则属性将被完全删除，除非上下文被显式更改为其他内容。
...
如果属性的值 `class` 碰巧为空，HTML模板语言将自动从输出中删 `class` 除整个属性。
...
对于设置属性， [`data-sly-attribute`](block-statements.md#attribute) 该语句可能也很有用。
...
在上面的第二个示例中，如果HTML和 **`head`** 元 **`body`** 素被放置到不同的文件中，则 **`clientlib.html`** 模板随后必须加载到需要它的每个文件中。

模板和调用语 [句的部分](block-statements.md#template-call) ，提供了有关声明和调用此类模板的工作方式的更多详细信息。
...
一个特殊情况是编写位于脚本元素 [中的客户端模板](getting-started.md#lifting-limitations-of-special-contexts) （如Handlebars），该特殊情况可以合法地使用特殊上下文的提升限制 **部分** 。 这种情况下可以安全地使用此技术的原因是 **脚本** 元素不像假定的那样包含JavaScript，而是包含更多HTML元素。 下面是一个如何实现该功能的示例：
...
如上所示，将包含在元素中的标 **`script`** 记可以包含HTL块语句，而表达式无需提供显式上下文，因为Handlebars模板的内容已在其自己的文件中隔离。 此外，此示例还说明如何将服务器端执行的HTL(如 **`h2`** 元素)与客户端执行的模板语言（如Handlebars） **`h3`** 混合。

但是，更现代的技巧是改用HTML元 **`template`** 素，因为这样做的好处是不需要将模板的内容分离为单独的文件。

**阅读下一篇文章：**

* [表达式语](expression-language.md) -详细了解在HTL表达式中可以执行哪些操作。
