...
现在，为该组件添加了一些内容，以便在 `/content/my-example/` 上呈现：
...
当用户访问此内容时，系统会执行 HTL 文件。在 HTL 代码内，使用上下文对象 `properties` 来访问当前资源的 `title` 和 `description`，并显示它们。 输出文件 `/content/my-example.html` 为：
...
例如，假设您想让 `info` 组件显示资源的 `title` 和 `description` 属性且必须全部小写。因为 HTL 没有用于小写字符串的方法，所以您需要一个使用类，通过添加 Java 使用类并更改  `/apps/my-example/component/info/info.html`  如下：
...
使用本地安装时，使用类的包名称必须与存储库文件夹位置匹配。将路径中的所有连字符替换为包名称中的下划线。
...
`data-sly-use` 属性用于初始化 HTL 代码中的 use 类。在此示例中，`data-sly-use` 属性声明我们需要使用类 `Info`：在此案例中，你可以只使用该类的局部名称，因为你使用的是局部安装（Java 源文件与 HTL 文件放在同一文件夹中）。如果我们使用捆绑安装，则必须指定完全限定的类名称。
